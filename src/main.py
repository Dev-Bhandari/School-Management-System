...
h = 2
w = 300
db = ms.connect(host = "localhost", user = os.environ.get("Mysql_user"),
                password = os.environ.get("Mysql_pass"), database='project')
cur = db.cursor()
screen1 = Tk()
...
def studentlogin():
    ...
    frame1 = LabelFrame(screen1, width=600, height=400,
                        bg='white').place(x=650, y=370)
    ...
    Label(frame1, text="User ID :", font=(
        "Calibri", 18), bg='white').place(x=700, y=470)
    username_entry = Entry(frame1, textvariable=username,
                           width="35", font=("Calibri", 16), bg='grey70').place(x=700, y=520)
    Label(frame1, text="Ex: 20012345", font=(
        "Calibri", 14), bg='white').place(x=1100, y=515)

    Label(frame1, text="Password :", font=(
        "Calibri", 18), bg='white').place(x=700, y=570)
    password_entry = Entry(frame1, textvariable=password,
                           show="*", width="35", font=("Calibri", 16), bg='grey70').place(x=700, y=615)
    Label(frame1, text="Ex: 21-OCT-2003", font=("Calibri", 14),
          bg='white').place(x=1100, y=610)
...
def staff_login():
    ...
    def profile():
        Label(screen2, width=500, height=500,
              bg='deepskyblue4').place(x=500, y=200)
    ...
    def add_staff():
        def add():
            ...
            data = (add_name.get(), add_userid.get(),
                    add_password.get(), add_sub.get(), add_email.get())
        ...
        Label(screen2, width=500, height=500,
              bg='deepskyblue4').place(x=500, y=300)
    ...
    def add_st():
        def add():
            ...
            data = (add_adm.get(), add_stname.get(), add_cl.get(),
                    add_sec.get(), add_phone.get(), add_dob.get(), add_email.get())
            cur.execute(add, data)
            db.commit()
            add_adm.set('')
            add_stname.set('')
    ...
    def existing():
        ...
        def delete():
            msg = messagebox.askquestion(
                'Delete Details', 'Are you sure you want to delete all the details ?', icon='warning')
            if(msg == 'yes'):
                string = (search1.get(),)
    ...
    def search():
        ...
        search1 = StringVar()
        Label(screen2, width=500, height=500,
              bg='deepskyblue4').place(x=500, y=150)
    ...
    def logout():
        msg = messagebox.askquestion(
            'Logout', 'Are you sure you want to logout ?', icon='warning')
        if(msg == 'yes'):
...
def std_login():
    ...
    def profile():
        Label(screen2, width=500, height=300,
              bg='deepskyblue4').place(x=400, y=200)
